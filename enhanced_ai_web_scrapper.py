...
PROMETHUS_PORT = int(os.getenv("PROMETHEUS_PORT", 8000))
CACHE_EXP = int(os.getenv("CACHE_EXPIRY", 3600))
...
async def analyze_content(url, headless=True):
    ...
    try:
        scraper = WebsiteScraper(headless=headless)
        ...
        if USE_OPENAI:
            ...
            if not response["message"]:
                raise AnalysisError("Empty response from Ollama")
